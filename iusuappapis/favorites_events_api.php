<?php 
 ...
 case 'get_favorites':
    if(isset($_GET['regNo'])){
        ...
        try{
        $stmt = $conn->prepare("SELECT fav.favoriteId, fav.regNo, fav.postId, post.image, post.title, post.description, DATE_FORMAT(post.date_Time,'%D %M,%Y'),  DATE_FORMAT(post.date_Time,'%H:%i'),post.category,post.venue, post.guildOfficialId, gp.title FROM favoritetb fav left outer join  posttb post on fav.postId=post.postId left outer join guildofficialtb go on post.guildOfficialId=go.guildOfficialId left outer join guildposttb gp on go.guildPostId=gp.guildPostId WHERE post.category = 'Event' and fav.regNo = ?  order by post.postId DESC ");
