<?php 
 ...
 case 'get_favorites':
    if(isset($_GET['regNo'])){
        ...
        try{
        $stmt = $conn->prepare("SELECT fav.favorite_id, fav.reg_no, fav.post_id, post.image, post.title, post.description, DATE_FORMAT(post.date_time,'%D %M,%Y'), post.category, post.guild_official_id, gp.title FROM favoritetb fav left outer join  post_tb post on fav.post_id=post.post_id left outer join guild_official_tb go on post.guild_official_id=go.guild_official_id left outer join guild_posts_tb gp on go.guild_post_id=gp.guild_post_id WHERE post.category = 'News' and fav.reg_no = ?  order by post.post_id DESC ");
