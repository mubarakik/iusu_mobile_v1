<?php 
 ...
 $stmt = $conn->prepare("INSERT INTO post_tb (image,title,description,date_time,category,guild_official_id,venue) VALUES (?,?,?,STR_TO_DATE(?, '%d %M, %Y %h:%i'),?,?,?)");
 ...
 $stmt = $conn->prepare("SELECT post.post_id, post.image, post.title, post.description, DATE_FORMAT(post.date_time,'%D %M,%Y'), DATE_FORMAT(post.date_time,'%H:%i'),post.category, post.venue,post.guild_official_id, gp.title FROM post_tb post left outer join guild_official_tb go on post.guild_official_id=go.guild_official_id left outer join guild_posts_tb gp on go.guild_post_id=gp.guild_post_id WHERE post.category = 'Event' order by post.post_id DESC ");
 $stmt->execute();
...
echo json_encode($images);
 ...
 try{
  ...
  $stmt = $conn->prepare("UPDATE post_tb SET title=?,description=?, image=?, date_time=? ,venue=? where post_id=?");
 ...
 $stmt = $conn->prepare("SELECT post.post_id, post.image, post.title, post.description, DATE_FORMAT(post.date_time,'%D %M,%Y'), DATE_FORMAT(post.date_time,'%H:%i'),post.category, post.venue,post.guild_official_id, gp.title FROM post_tb post left outer join guild_official_tb go on post.guild_official_id=go.guild_official_id left outer join guild_posts_tb gp on go.guild_post_id=gp.guild_post_id WHERE post.category = 'Event' order by post.post_id DESC LIMIT 4 ");
 $stmt->execute();
